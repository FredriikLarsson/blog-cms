...
if (empty($title)) {
    ...
    header("Location: http://localhost/Projekt_Blogg/create_post.php?noBlogTitle=true");
    exit();
}
...
if (createPost($title, $content, $imageFilePath, $userId)) {
    ...
    header("Location: http://localhost/Projekt_Blogg/admin.php?error=postNotCreated");
    exit();
}
