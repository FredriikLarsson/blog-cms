...
if (getUserBlog($_SESSION['userId']) == false) {
    if (!file_exists('../uploads/' . $_SESSION['userId'])) {
        mkdir('../uploads/' . $_SESSION['userId'], 0777, true);
    }
    ...
    if (file_exists($_FILES['blog-image']['tmp_name'])) {
        ...
        $upload_dir = '../uploads/'  . $_SESSION['userId'] . '/'; //Directory on server that image is going to be stored in
    ...
    if (file_exists($_FILES['image']['tmp_name'])) {
        ...
        //check if image upload went through
        if (move_uploaded_file($tmp_file, $upload_dir . $target_file)) {
            $upload_dir_absolute = "/~frelab-8/Projekt_Blogg/uploads/" . $_SESSION['userId'] . '/'; //Directory on server that image is going to be stored in
